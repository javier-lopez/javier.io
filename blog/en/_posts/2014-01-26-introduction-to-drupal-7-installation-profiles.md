...
Last weekend I worked on a Drupal site and it was not fun =( mostly due to incomplete and inaccurate documentation.

My goal was to create a distributable bundle for Spanish Ubuntu local teams. I already had a base theme and didn't though it could be that hard, however what started as a simple task ended as a very long weekend. On this post I'll try to document which issues I found and how they were worked out.
...
1. Download and place the bundle in a directory read by a web server (eg, apache)
2. Go to the server ip
3. Select the 'Ubuntu-mx' profile
4. On a Spanish translated interface, input the database settings
5. Configure the administrator credentials
6. Create content

The resulting site would be translated to spanish, will enable a bunch of modules (l10n_update, admin_menu, smtp, ckeditor), an Ubuntu theme, search permitions and menu links.
...
Installation profiles are special modules which allow you to declare which modules, languages and themes to load by default, they can also be used to modify the installer itself to add or remove steps.
...
On the *.profile* and *.install* files can be written functions to override/define hooks, I used **/profile/umx/umx.profile** to configure the default language (which as result skips the language dialogue):
...
The localize.drupal.org server has po files with a drupal version prefix, if downloaded from there, you should rename it. Eg
...
On the install file you can override install, update and uninstall functions. Since the feature modules doesn't support theme settings (which would be really useful) I declared the default theme on this file (**/profile/umx/umx.install**):
...
According to documentation, theme settings should be declared in **profile_themes_enabled()** using the **theme_enable()** function, however I was unable to make to work any of them.
...
As I already commented, I used the installation profile for general definition, however bundling configurations, permissions and dependencies on non default modules where saved on a feature module (umx_conf), this allowed me to iterate faster because I could modify the site using a browser and recreate the module through the feature menu.

Features modules can contain the following files:

- *.info*
- *.module*
- *.feature.submodule.inc*

These files are generated automatically, very few times you would like to modify them directly. In my experience (a weekend) sometimes the autogenerated modules contain mistakes, therefore I needed to modify them slighly to feet my needs:
...
However I got a hard time trying to disable the **toolbar** module after enabling **admin_menu**. At the end I resolved to delete the toolbar dependency from all my *info* files (/profiles/umx/umx.info and /sites/all/modules/umx_conf/umx_conf.info).
...
Here specific configuration for each module are saved, for my case, I had to modify **/sites/all/modules/umx_conf/umx_conf.features.menu_links.inc** because some links weren't saved by the autogenerator.
...
I had to adjust mostly the **weight** parameter and add some missing links

- [Introduction to drupal module features](http://www.youtube.com/watch?v=DxRBEaD9JCA)

That's it, hope this information can save some hours to you, have fun!
